...
class QMatrix (object):
    ...
    @classmethod
    def zeros(cls, n,m):
        return QMatrix(P=np.zeros((n,m), dtype=np.int64), Q=np.ones((n,m), dtype=np.int64))
        
    @classmethod
    def ones(cls, n,m):
        return QMatrix(P=np.ones((n,m), dtype=np.int64), Q=np.ones((n,m), dtype=np.int64))
    ...
    def __getitem__(self, key):
        ...
        if isinstance(ki, int) and isinstance(kj, int):
            res = Fraction(p, q)
        elif not isinstance(ki, int) and isinstance(kj, int): # Selection of the column kj
            n = len(p)
    ...
    def __add__(self, b):
        ...
        if not isinstance(b,QMatrix) and not hasattr(b,'__iter__'):
            p0 = b.numerator
            q0 = b.denominator
            b = QMatrix(P=p0*np.eye(n, dtype=np.int64), Q=q0*np.ones((n,n), dtype=np.int64))
        ...
        for r in range(n): # r décrit tous les indices de colonnes
            ...
            for i in range(n): # On simplifie les autres lignes
                if i != r:
                    ...
                    if debug:
                        print('i,dL', i,PM[i,:]/QM[i,:]-PM[r,:]/QM[r,:]*PM[i,r]/QM[i,r])
