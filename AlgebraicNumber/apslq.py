...
def apslq(xx : np.array, iterations : int = 1000, digits : int = 100, gmma : float = 2/np.sqrt(3), threshold : float = 10**(-12), D : int = 0) -> np.array:
    '''
    APSLQ over Q[sqrt(-D)]
    ...
    def AlgNearest(x, D):
        ...
        if -D == 0 or -D == 1:
            ...
            candidates = np.array([ a[0] + b*omega, a[1] + (b+1)*omega ], dtype=np.complex64)
    ...
    miny = np.abs( yy[pos] )
    if miny < threshold: # No need to normalize because B is the identity matrix, so miny / ||Col(B,pos)|| = miny.
        return B[:,pos]
    ...
    for ii in range(iterations):
        ...
        if m < n-2:
            delta = np.sqrt( H[m,m]*np.conj(H[m,m]) + H[m,m+1]*np.conj(H[m,m+1]) )
            beta = H[m,m]
            lbd = H[m,m+1]
            for i in range(m,n):
        ...
        if np.abs(H[n-2,n-2]) < threshold:
            print( "[WARNING]Found relation due to H[n-1,n-1]=0, which somehow got missed by the miny check." )
            return candidateRelation
